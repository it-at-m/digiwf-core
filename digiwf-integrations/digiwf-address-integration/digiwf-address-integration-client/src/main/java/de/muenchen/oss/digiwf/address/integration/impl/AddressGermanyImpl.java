package de.muenchen.oss.digiwf.address.integration.impl;

import de.muenchen.oss.digiwf.address.integration.api.AddressGermanyApi;
import de.muenchen.oss.digiwf.address.integration.exception.AddressServiceIntegrationClientErrorException;
import de.muenchen.oss.digiwf.address.integration.exception.AddressServiceIntegrationException;
import de.muenchen.oss.digiwf.address.integration.exception.AddressServiceIntegrationServerErrorException;
import de.muenchen.oss.digiwf.address.integration.gen.api.AdressenBundesweitApi;
import de.muenchen.oss.digiwf.address.integration.gen.model.BundesweiteAdresseResponse;
import de.muenchen.oss.digiwf.address.integration.model.request.SearchAdressesGermanyModel;
import lombok.RequiredArgsConstructor;
import lombok.extern.slf4j.Slf4j;
import org.springframework.web.client.HttpClientErrorException;
import org.springframework.web.client.HttpServerErrorException;
import org.springframework.web.client.RestClientException;

@RequiredArgsConstructor
@Slf4j
public class AddressGermanyImpl implements AddressGermanyApi {

    private final AdressenBundesweitApi adressenBundesweitApi;

    @Override
    public BundesweiteAdresseResponse searchAddresses(final SearchAdressesGermanyModel searchAdressesGermanyModel) throws AddressServiceIntegrationClientErrorException, AddressServiceIntegrationServerErrorException, AddressServiceIntegrationException {
        try {
            return this.adressenBundesweitApi.searchAdressen(
                    searchAdressesGermanyModel.getQuery(),
                    searchAdressesGermanyModel.getZip(),
                    searchAdressesGermanyModel.getCityName(),
                    searchAdressesGermanyModel.getGemeindeschluessel(),
                    searchAdressesGermanyModel.getHouseNumberFilter(),
                    searchAdressesGermanyModel.getLetterFilter(),
                    searchAdressesGermanyModel.getSort(),
                    searchAdressesGermanyModel.getSortdir(),
                    searchAdressesGermanyModel.getPage(),
                    searchAdressesGermanyModel.getPagesize()
            );
        } catch (final HttpClientErrorException exception) {
            final String message = String.format("The request to get address bundesweit failed with %s.", exception.getStatusCode());
            log.error(exception.getMessage());
            log.error(message);
            throw new AddressServiceIntegrationClientErrorException(message, exception);
        } catch (final HttpServerErrorException exception) {
            final String message = String.format("The request to get address bundesweit failed with %s.", exception.getStatusCode());
            log.error(exception.getMessage());
            log.error(message);
            throw new AddressServiceIntegrationServerErrorException(message, exception);
        } catch (final RestClientException exception) {
            final String message = String.format("The request to get address bundesweit failed.");
            log.error(exception.getMessage());
            log.error(message);
            throw new AddressServiceIntegrationException(message, exception);
        }
    }

}
