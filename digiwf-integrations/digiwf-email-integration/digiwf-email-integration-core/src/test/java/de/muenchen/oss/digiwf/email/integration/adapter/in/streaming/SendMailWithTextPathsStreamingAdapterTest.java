...
class SendMailWithTextPathsStreamingAdapterTest extends StreamingAdapterTestBase {
    ...
    private Message<TextMailPaths> message;

    @BeforeEach
    void setup() {
        setupBase();
        this.message = new Message<TextMailPaths>() {
            @Override
            public TextMailPaths getPayload() {
                return mail;
            }
    ...
    void testEmailIntegrationSendsMailSuccessfully() {
        streamingAdapter.sendTextMailV2().accept(this.message);
        verify(monitoringServiceMock, times(1)).sendMailSucceeded();
        verify(sendMailPathsInPortMock, times(1)).sendMailWithText(mail);
    }

    @Test
    void testEmailIntegrationHandlesValidationException() {
        Mockito.doThrow(new ValidationException("Test ValidationException")).when(sendMailPathsInPortMock).sendMailWithText(any());
        streamingAdapter.sendTextMailV2().accept(this.message);
    ...
    void testEmailIntegrationHandlesBpmnError() {
        Mockito.doThrow(new BpmnError("errorCode", "errorMessage")).when(sendMailPathsInPortMock).sendMailWithText(any());
        streamingAdapter.sendTextMailV2().accept(this.message);
        verify(monitoringServiceMock, times(1)).sendMailFailed();
        final ArgumentCaptor<Map> messageHeaderArgumentCaptor = ArgumentCaptor.forClass(Map.class);
        verify(errorApiMock, times(1)).handleBpmnError(messageHeaderArgumentCaptor.capture(), any(BpmnError.class));
    ...
    void testEmailIntegrationHandlesIncidentError() {
        Mockito.doThrow(new IncidentError("Error Message")).when(sendMailPathsInPortMock).sendMailWithText(any());
        streamingAdapter.sendTextMailV2().accept(this.message);
        verify(monitoringServiceMock, times(1)).sendMailFailed();
        final ArgumentCaptor<Map> messageHeaderArgumentCaptor = ArgumentCaptor.forClass(Map.class);
