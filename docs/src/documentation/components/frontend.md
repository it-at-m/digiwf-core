...
Für die Authentication wird eine Session, welche im Api Gateway verwaltet wird, verwendet. Der JWT Token von Keycloak wird nicht an den Client weitergereicht. 
...
Beim Starten der Anwendung zur lokalen Entwicklung muss man als Erstes das Api Gateway (http://localhost:8081) aufrufen (1). 
...
Jeder Netzwerkrequest vom Browser zum Api Gateway liefert die Cookies und somit die Session mit. Um Cors zu vermeiden wird bei jedem Netzwerkrequest der Proxy Server des Vite Development Servers angesprochen (5). Dieser leitet den Request unverarbeitet weiter zum Api Gateway (6).
...
In den folgenden Diagram ist der Ablauf der Netzwerkrequests bei einem Session Timeout beschrieben.

![Session timeout in frontend](~@source/images/platform/components/frontend/session-timeoutapi-gateway-frontend.png)

Der Browser sendet einen Api Request mit abgelaufener Session an das den Vite Proxy (1). Dieser leitet den Request unverändert an das Api Gateway weiter (2). 
Bei der Überprüfung im Api Gateway wird festgestellt, dass die Session abgelaufen ist. Daraufhin wird eine Response mit Status Code 302 und _location_ als Header an den Client gesendet (4,5). In der _location_ steht der Zielpfad für den erneuten Aufruf statt. Dieser wird automatisch vom Browser aufgerufen (6) und endet mit einer Response mit Status Code 404 beantwortet.
Deswegen wird in Axios eine Exception geworfen und der global definierte Error Interceptor ausgeführt. Dieser überprüft im Fehlerfall über die Ziel- ist Ist-URL des gescheiterten Requests. Sollten diese nicht übereinstimmen, kann davon ausgegangen werden, dass eine Weiterleitung stattfand (7). 
