...
export const validateTime = (
  time: string = "",
  nativeValidationResult: boolean = true
): string | boolean => {
  ...
  if (isNaN(dateObject)) {
    ...
    if (dateObject.invalid.reason === "unit out of range") {
      if (
        dateObject.invalid.explanation.includes("as a minute, which is invalid")
      ) {
        return ERROR_MESSAGES.MINUTES_TO_HIGH;
      }
      if (
        dateObject.invalid.explanation.includes("as a hour, which is invalid")
      ) {
