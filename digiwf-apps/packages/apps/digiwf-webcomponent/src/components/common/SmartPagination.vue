<template>
  <c-pagination :size="paginationSize" :align="align">
    <c-pagination-item v-if="showLeftRightButtons" component="button" :disabled="isOnFirstPage" @click="gotoPreviousPage">
      <svg-icon
        type="mdi"
    ...
    <template v-if="showPageButtons">
      <c-pagination-item v-for="page in visiblePages" :key="page" component="button" :active="isActivePage(page)" @click="gotoPage(page - 1)">{{ page }}</c-pagination-item>
    </template>
    <c-pagination-item v-if="showLeftRightButtons" component="button"  :disabled="isOnLastPage" @click="gotoNextPage">
      <svg-icon
        type="mdi"
...
const visiblePages = computed(() => {
  ...
  const rangeStart = Math.max(1, (props.activePage + 1) - Math.floor(props.amountPagesVisible / 2));
  const rangeEnd = Math.min(props.amountPages, rangeStart + props.amountPagesVisible - 1);
  ...
  if (rangeEnd - rangeStart + 1 < props.amountPagesVisible) {
    ...
    return Array.from({ length: props.amountPagesVisible }, (_, i) => i + rangeStart - Math.min(offset, rangeStart - 1));
  }
...
const paginationSize = computed(() => props.size ?? "")
