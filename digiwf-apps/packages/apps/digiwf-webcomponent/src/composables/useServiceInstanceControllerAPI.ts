...
export function useServiceInstanceControllerAPI() {
  ...
  const serviceInstanceControllerAPI = inject(SERVICE_INSTANCE_CONTROLLER_API_INJECT_KEY);

  const callGetAssignedProcessInstances = async (page: number, size: number, query?: string): Promise<PageServiceInstanceTO | undefined> => {
    const service = serviceInstanceControllerAPI!.value;

    try {
      const result = await service.getAssignedInstances(page, size, query, FetchUtils.getGETConfig());
      return result.data
    } catch (error) {
