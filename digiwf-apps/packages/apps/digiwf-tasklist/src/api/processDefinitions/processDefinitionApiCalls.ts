...
export const callGetProcessDefinitionsFromEngine = (
  page: number,
  size: number,
  query?: string
): Promise<PageServiceDefinitionTO> => {
  const cfg = ApiConfig.getAxiosConfig(FetchUtils.getGETConfig());

  return ServiceDefinitionControllerApiFactory(cfg)
    .getServiceDefinitions(page, size, query)
...
export const callGetProcessDefinition = (
  processKey: string
): Promise<ServiceDefinitionDetailTO> => {
  const cfg = ApiConfig.getAxiosConfig(FetchUtils.getGETConfig());
  return ServiceDefinitionControllerApiFactory(cfg)
    ...
    .catch((e: AxiosError) => {
      ...
      if (status === 403) {
        return Promise.reject(
          "Es liegt keine Berechtigung zum Starten dieses Vorgangs vor. " +
            "Weitere Infos finden Sie in unseren FAQs in Wilma."
        );
      }
      return Promise.reject(
        "Der Vorgang konnte nicht geladen werden. Status Code: " + status
      );
    });
};
