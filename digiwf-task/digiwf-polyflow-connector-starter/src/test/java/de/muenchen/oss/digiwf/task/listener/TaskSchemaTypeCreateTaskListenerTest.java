...
class TaskSchemaTypeCreateTaskListenerTest {
    ...
    public void detect_schema_based_form() {
        ...
        assertThat(task.getVariablesLocal()).containsEntry(TaskVariables.TASK_SCHEMA_TYPE.getName(), TaskSchemaType.SCHEMA_BASED);
    ...
    public void detect_vuetify_form_base_form() {
        ...
        listener.taskCreated(task);
        assertThat(task.getVariablesLocal()).containsEntry(TaskVariables.TASK_SCHEMA_TYPE.getName(), TaskSchemaType.VUETIFY_FORM_BASE);
        assertThat(task.getVariablesLocal()).containsEntry(TaskVariables.TASK_SCHEMA_KEY.getName(), "form-ref");
        verify(formService).getTaskFormKey(task.getProcessDefinitionId(), task.getTaskDefinitionKey());
    }
