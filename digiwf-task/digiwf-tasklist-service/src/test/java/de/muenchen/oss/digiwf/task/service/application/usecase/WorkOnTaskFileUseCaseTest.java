...
class WorkOnTaskFileUseCaseTest {
    ...
    @Test
    void getFileNamesDocumentWithStorageUrl()
    ...
    @Test
    void getFileNamesDocumentWithoutStorageUrl()
            throws DocumentStorageException, DocumentStorageClientErrorException, DocumentStorageServerErrorException {
        final Set<String> filesInS3 = Sets.newHashSet(fileContext + "/able/to/read/file1.txt", fileContext + "/able/to/read/secondfile1.pdf");
    ...
    @Test
    void getFileNames_DoesNotIncludeFilesFromSubdirectories()
            throws DocumentStorageException, DocumentStorageClientErrorException, DocumentStorageServerErrorException {
    ...
    @Test
    void getFileNamesThrowsException()
            throws DocumentStorageException, DocumentStorageClientErrorException, DocumentStorageServerErrorException {
        when(taskFileConfigResolverPort.apply(any())).thenReturn(new TaskFileConfig(this.fileContext, "asyncConfig", "syncConfig", pathsWrite, pathsReadonly));
        when(documentStorageFolderRepository.getAllFilesInFolderRecursively(anyString(), anyString())).thenThrow(
                new DocumentStorageException("DocumentStorageException", new Exception()));
    ...
    void getPresignedUrlForPUTWithReadAccess() {
        when(taskFileConfigResolverPort.apply(any())).thenReturn(
                new TaskFileConfig(this.fileContext, "asyncConfig", "syncConfig", pathsWrite, List.of("readonly/path")));
        when(presignedUrlPort.getPresignedUrl(anyString(), anyString(), anyInt(), any())).thenReturn("With DocumentStorageUrl");
