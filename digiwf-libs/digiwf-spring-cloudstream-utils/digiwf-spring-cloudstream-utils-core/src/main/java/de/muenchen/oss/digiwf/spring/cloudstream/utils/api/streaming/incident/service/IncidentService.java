package de.muenchen.oss.digiwf.spring.cloudstream.utils.api.streaming.incident.service;
...
public class IncidentService {
    ...
    public boolean sendIncident(final MessageHeaders messageHeaders, final String errorMessage) {
        final Message<String> message = MessageBuilder
                ...
                .setHeader(StreamingHeaders.DIGIWF_PROCESS_INSTANCE_ID, messageHeaders.get(StreamingHeaders.DIGIWF_PROCESS_INSTANCE_ID))
                // bpmn token is waiting on that message receive event. Thats where we want our incident to be placed:
                .setHeader(StreamingHeaders.DIGIWF_MESSAGE_NAME, messageHeaders.get(StreamingHeaders.DIGIWF_MESSAGE_NAME))
                .build();
